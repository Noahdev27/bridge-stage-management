...
import { createRh, type RhActionState } from "../actions";

export function RhCreateForm() {
  const router = useRouter();
  const { showToast } = useToast();

  const [state, formAction, isPending] = useActionState<RhActionState, FormData>(
    createRh,
    {}
  );
  ...
  useEffect(() => {
    ...
    showToast({ type: "success", message: "Compte RH créé avec succès." });
    ...
    <div className="max-w-2xl mx-auto space-y-6">
      <div>
        ...
        >
          ...
          Retour aux comptes RH
        </Link>
        <h1 className="text-3xl font-bold tracking-tight mt-3">
          Nouveau compte RH
        </h1>
        <p className="text-base-content/60 mt-1">
          Ce compte aura les mêmes droits que le vôtre : traiter les dossiers,
          gérer les offres, les tuteurs et créer d&apos;autres comptes RH.
        </p>
      </div>
        ...
        <div className="card-body p-5 gap-4">
          ...
          <label className="form-control">
            ...
            {fieldErrors.password ? (
              ...
              <span className="label-text-alt text-base-content/50 mt-1">
                À communiquer au titulaire par un canal sûr. Il pourra le changer
                depuis son espace Paramètres.
              </span>
            )}
          </label>
          ...
          <div className="flex flex-col-reverse sm:flex-row gap-2 pt-2 border-t border-base-200">
            ...
            >
              ...
              {isPending ? "Création…" : "Créer le compte RH"}
            </button>
          </div>
        </div>
      </form>
